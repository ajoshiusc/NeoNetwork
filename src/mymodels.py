...
__all__ = ["SegResNetLatentOut"]


class SegResNetLatentOut(nn.Module):
    """
    ...
    def __init__(
        ...
        # add fully connected layer to predict the age from latent space
        self.fc = nn.Linear(32768,1)#4*4*4*init_filters, 1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # apply fully connected layer to predict the age
        x_encoded = x_encoded.view(x_encoded.size(0), -1)
        x_age = self.fc(x_encoded)
    ...
    # use the latent as input to a regression model to predict the age
    # the regression model is a simple linear layer
    def get_age(self, x: torch.Tensor) -> torch.Tensor:
        #x = self.get_latent(x)
        #x = x.view(x.size(0), -1)
        x = self.fc(x)
        return x
...
# add main to the module for testing
